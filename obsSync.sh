...
# $0 - содержит путь, которым был вызван скрипт
# readling -f "$0" - получаем абсолютный путь
# Скрипт для безопасного добавления автозапуска
SCRIPT_PATH="$(readlink -f "$0")"
LOG_PATH="/dev/null"
CRON_JOB="@reboot $SCRIPT_PATH > $LOG_PATH 2>&1"
